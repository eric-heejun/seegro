...
type SalesChannel = "cafe24" | "naver";

const SALES_CHANNELS: { id: SalesChannel; label: string }[] = [
...
export default function Dashboard() {
  const [salesChannel, setSalesChannel] = useState<SalesChannel>("cafe24");
  ...
  useEffect(() => {
    if (salesChannel !== "cafe24") {
      setShops([{ shop_no: 0, shop_name: "네이버" }]);
      return;
    }
  ...
  useEffect(() => {
    ...
    async function loadOrders() {
      ...
      try {
        ...
        if (salesChannel === "cafe24") {
          params.set("shop_no", shopNo);
        }

        const endpoint =
          salesChannel === "naver" ? "/api/naver/orders" : "/api/cafe24/orders";
  ...
  }, [salesChannel, shopNo, dateRange.startDate, dateRange.endDate]);
  ...
  useEffect(() => {
    ...
    async function loadAdInsights() {
      ...
      try {
        ...
        const response = await fetch(`/dashboard-performance/meta-insights?${params}`, {
          signal: controller.signal
        });
        const payload = await readJsonResponse(response);
        if (controller.signal.aborted) {
    ...
    <main className="dashboardShell">
      ...
      <section className="dashboardControls">
        <div className="controlGroup">
          <label htmlFor="sales_channel">판매채널</label>
          <select
            id="sales_channel"
            value={salesChannel}
            onChange={(event) => {
              setSalesChannel(event.target.value as SalesChannel);
              setShopNo("all");
            }}
          >
